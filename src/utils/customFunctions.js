...
export const getDeliveryFees = (
    ...
    //     return 0
    // }else{
    //     const result =  cartList.reduce((total, item) => total + parseFloat(item.plans?.package?.delivery_charge ?? 0), 0);
    //      return result ?? 0
    // }

    if (orderType?.length > 0 && Array.isArray(orderType)) {

        const totalDeliveryCharges = cartList.reduce((total, item) => total + parseFloat(item.plans?.package?.delivery_charge ?? 0), 0);
        // const finalTotal = orderType.every(type => type === 'delivery') ? totalDeliveryCharges : totalDeliveryCharges - 500;
        let final = 0;

        if (orderType.every(type => type === 'delivery')) {
            final += totalDeliveryCharges;
        } else if (orderType.every(type => type === 'take_away')) {
            final = 0;
        } else  {

            // const deliveryCount =  orderType.filter(e => e === 'delivery')?.length;
            // const takeawayCount =  orderType.filter(e => e === 'take_away')?.length;
            // if(deliveryCount > 0){
            //     for(let i=0; i<deliveryCount;i++){
            //         final += parseFloat(cartList[i].plans?.package?.delivery_charge ?? 0) 
            //     }
            // }

            orderType?.map((type, index)=> {
                if(type === 'delivery'){
                    final += parseFloat(cartList[index].plans?.package?.delivery_charge ?? 0) 
                }
            })

        }
        return final ?? 0;
    }else if (restaurantData?.data?.free_delivery || orderType === 'take_away') {
        return 0
    }else{
        const result = cartList.reduce((total, item) => total + parseFloat(item.plans?.package?.delivery_charge ?? 0), 0);
        return result ?? 0
...
export const getDeliveryFees2 = (
    ...
    if (orderType?.length > 0) {
        let final = 0;
        orderType.forEach((value, i) => {
            if (value === 'delivery') {
                final = cartList.reduce((total, item) => total + parseFloat(item.plans?.package?.delivery_charge ?? 0), 0);
            } else {
                final = cartList.reduce((total, item) => total - parseFloat(cartList[i]?.plans?.package?.delivery_charge ?? 0), 0);
            }
        });
        return final ?? 0;
    }
