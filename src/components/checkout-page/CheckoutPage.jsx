...
const CheckoutPage = () => {
    ...
    useEffect(()=> {
        setAddress2(Array(cartList?.length).fill(''))
    },[cartList])
    ...
    const [addresValid,setaddressValid] = useState(false)
    ...
    useEffect(()=> {
        if(cartList?.length > 0 ){
            setOrderType2(Array(cartList?.length).fill('delivery'))
        }
    },[cartList])
    ...
    const orderPlaceMutation = async (
        ...
        for (let i = 0; i < carts?.length; i++) {
            ...
            await new Promise((resolve) => {
                setTimeout(() => {
                    let order = handleOrderMutationObject(carts[i],address2[i],orderType2[i], productList[i], carts?.length)
                    orderMutation(order, {
                        onSuccess: handleSuccess,
                        onError: (error) => {
                            error?.response?.data?.errors?.forEach((item) =>
    ...
    useEffect(()=> { 
        let condi = false
        for(let i = 0; i<orderType2?.length; i++){
            if(orderType2[i] == 'delivery'){
                if( address2[i] === '') {
                    condi = false;
                    break;
                }else{
             ...
             }else if (orderType2[i] == 'take_away'){
                 if( address2[i] === '') {
                     condi = true
                 }else{
                     condi = true
            ...
            if (subscriptionStates.order === '1') {
                ...
                if (
                    ...
                    if(addresValid){
                        ...
                        toast(t('You must be select address'), {
                            duration: 4000,
                            icon: '⚠️',
                            style: {
                                textTransform: 'none',
    ...
    return (
        <Grid container spacing={3} mb="2rem" mt="2rem">
            ...
            <section className=''>
                ...
                <CustomPaperBigCard height="auto">
                    <Stack spacing={3} justifyContent="space-between">
                        ...
                        <h4 className='fw-bold fs-20'>{t('Order Summary')}</h4>
                        {zoneData &&
                        ...
                        {distanceData && restaurantData ? (
                            <OrderCalculation
                                ...
                                setCouponDiscounts={setCouponDiscount}
                                counponRemoves={counponRemove}
